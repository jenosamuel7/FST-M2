-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/Jeno/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/Jeno/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/Jeno/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/Jeno/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/Jeno/outputs' USING PigStorage('\t');
